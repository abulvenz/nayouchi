...
const exists = (val, fn, def) => val ?
  (typeof fn === 'function' ? fn(val) : fn) :
  (def ? def : null);
 ...
 *                
 *    to separate the common components                                                   
 */
...
class InputWithEnter {
  ...
  view(vnode) {
    return m('.input-group',
      exists(vnode.attrs.icon, icon => m('span.input-group-addon',
        m(Icon, {
          icon: icon
        }))),
  ...
  view(vnode) {
    return vnode.attrs.connected ?
      m(SomeThingWithTooltip, m('.badge', {
        ...
        icon: 'ok-circle'
      })), m(Tooltip, 'Du bist verbunden zum Server.')) :
      m(SomeThingWithTooltip, m('.badge', {
        style: 'background-color:red'
      }, m(Icon, {
        icon: 'ban-circle'
      })), m(Tooltip, 'Du bist nicht verbunden zum Server.'));
  }
}
...
class User {
  ...
  createGroup(name) {
    bus.send('create', {
      name: name,
      usr: this.id
    }, (err, res) => this.enterGroup(res.body.grp))
  }
  addMember(email) {
    bus.send('addMember', {
      usr: this.id,
 ...
 *    (_______/(_______)|/     \||/     \|(_______/|/    )_)   )_(   
 *            
 *    to separate the custom components                                                       
 */
...
class NameList {
  view(vnode) {
    return m(ListGroup,
      exists(vnode.attrs.onadd, onadd => m(ListGroupItem,
        m(InputWithEnter, {
          icon: 'plus',
          onenter: n => onadd(n)
        }))),
      isNotEmpty(vnode.attrs.names) ?
      vnode.attrs.names.map(name_ => m(ListGroupItem, m(WikiLink, {
          link: name_
        }),
        exists(vnode.attrs.actions, actions => actions.map(action => {
...
class GroupView {
  view(vnode) {
    ...
    return m(ListGroupItem, {
        onclick: e => user.enterGroup(grp.id)
      },
      m(Icon, {
        icon: 'star'
      }), ' ',
...
class GroupListView {
  view(vnode) {
    return m(ListGroup,
      vnode.attrs.user.groups.map(group => m(GroupView, {
        group
      })),
      exists(vnode.attrs.onadd, onadd =>
        m(ListGroupItem,
          m(InputWithEnter, {
            icon: 'plus',
...
class InitiatorSign {
  view(vnode){
    return m(SomeThingWithTooltip, m(Icon, {
      icon: 'certificate'
    }), m(Tooltip, 'Dieser vielzackige Stern ist das Zeichen für die große Macht der Gruppenentscheider.')); 
  }
}
  ...
  view(vnode) {
    return [
      m(Heading, m(SomeThingWithTooltip, m('button.btn.btn-primary', {
        ...
        icon: 'arrow-left'
      })), m(Tooltip, 'Zurück zur Gruppenübersicht')), ' ', user.group.name),
      m(SubHeading, m(Icon, {
        icon: 'heart'
      }), ' ', m(SomeThingWithTooltip, 'Treffer', m(Tooltip, 'Treffer sind alle Übereinstimmungen zwischen den Entscheidern')), ' von ', user.group.initiators.join(', ')),
      m(NameList, {
    ...
    return m(Container,
      ...
      m('.alert.alert-success', 'Eine Email mit einem Speziallink wurde gesendet an ' + this.email) : [
       ...
       diese Applikation weiss
        ...
        Wer das nicht glaubt, kann bei `, m('a[href=https://github.com/abulvenz/nayouchi/tree/enterprise-edition]', m(Icon, {
            icon: 'link'
          }), 'github'), ` nachschauen.
        `))),
        m(Spacer, {
...
m.mount(document.body, {
  view: vnode => pathFracs.length > 1 ? [m(Layout, {
    user
  })] : [m(SignUp)]
});
